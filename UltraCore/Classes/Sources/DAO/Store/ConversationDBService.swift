...
class ConversationDBService {
    ...
    func createIfNotExist(from message: Message) -> Single<Void> {
        return Single.create { [weak self] observer -> Disposable in
            ...
            if let conversation = existConversation {
                self.updateConversationMessages(from: message, observer: observer)
            } else {
                    ...
                    .whenComplete { result in
                        ...
                        case .success(let response):
                            do {
                                ...
                                if localRealm.object(ofType: DBConversation.self, forPrimaryKey: message.receiver.chatID) != nil {
                                    self.updateConversationMessages(from: message, observer: observer)
                                    return
                                }
    ...
    func updateConversationMessages(from message: Message, observer: (Result<Void, Error>) -> Void) {
        let realm = Realm.myRealm()
        let peerID = message.peerId(user: userID)
