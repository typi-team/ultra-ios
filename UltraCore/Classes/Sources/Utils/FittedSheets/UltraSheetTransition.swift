...
public class UltraSheetTransition: NSObject, UIViewControllerAnimatedTransitioning {
    var presenting = true
    weak var presenter: UIViewController?
    var options: UltraSheetOptions
    ...
    init(options: UltraSheetOptions) {
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if self.presenting {
            guard let presenter = transitionContext.viewController(forKey: .from), let sheet = transitionContext.viewController(forKey: .to) as? UltraSheetViewController else {
                transitionContext.completeTransition(true)
                return
            }
            self.presenter = presenter
            
            if UltraSheetOptions.shrinkingNestedPresentingViewControllers {
                UltraSheetTransition.currentPresenters.append(presenter)
            }
            ...
            let sheet = transitionContext.viewController(forKey: .from) as? UltraSheetViewController else {
                transitionContext.completeTransition(true)
                return
            }
    ...
    func restorePresentor(_ presenter: UIViewController, animated: Bool = true, animations: (() -> Void)? = nil, completion: ((Bool) -> Void)? = nil) {
        UltraSheetTransition.currentPresenters.removeAll(where: { $0 == presenter })
        ...
        UIView.animate(
            ...
            animations: {
                ...
                if UltraSheetOptions.shrinkingNestedPresentingViewControllers {
                    ...
                    let presenters = UltraSheetTransition.currentPresenters.reversed()
    ...
    func setPresentor(percentComplete: CGFloat) {
        ...
        if UltraSheetOptions.shrinkingNestedPresentingViewControllers {
            let presenters = UltraSheetTransition.currentPresenters.reversed().dropFirst()
            for lowerPresenter in presenters {
                scale *= 0.92
