//
//  UltraSheetViewController.swift
//  FittedSheetsPod
//
...
public class UltraSheetViewController: UIViewController {
    public private(set) var options: UltraSheetOptions
    ...
    public var autoAdjustToKeyboard = UltraSheetViewController.autoAdjustToKeyboard
    ...
    public var allowPullingPastMaxHeight = UltraSheetViewController.allowPullingPastMaxHeight
	...
	public var allowPullingPastMinHeight = UltraSheetViewController.allowPullingPastMinHeight
    
    /// The sizes that the sheet will attempt to pin to. Defaults to intrinsic only.
    public var sizes: [UltraSheetSize] = [.intrinsic] {
        didSet {
            self.updateOrderedSizes()
        }
    }
    public var orderedSizes: [UltraSheetSize] = []
    public private(set) var currentSize: UltraSheetSize = .intrinsic
    ...
    public var blurEffect = UltraSheetViewController.blurEffect {
        didSet {
            self.blurView.effect = blurEffect
        }
    }
    public static var allowGestureThroughOverlay: Bool = false
    public var allowGestureThroughOverlay: Bool = UltraSheetViewController.allowGestureThroughOverlay {
        didSet {
    ...
    let transition: UltraSheetTransition
    
    public var shouldDismiss: ((UltraSheetViewController) -> Bool)?
    public var didDismiss: ((UltraSheetViewController) -> Void)?
    public var sizeChanged: ((UltraSheetViewController, UltraSheetSize, CGFloat) -> Void)?
    ...
    private var panGestureRecognizer: UltraInitialTouchPanGestureRecognizer!
    ...
    public init(controller: UIViewController, sizes: [UltraSheetSize] = [.intrinsic], options: UltraSheetOptions? = nil) {
        let options = options ?? UltraSheetOptions.default
        self.contentViewController = UltraSheetContentViewController(childViewController: controller, options: options)
        ...
        self.transition = UltraSheetTransition(options: options)
        self.minimumSpaceAbovePullBar = UltraSheetViewController.minimumSpaceAbovePullBar
        super.init(nibName: nil, bundle: nil)
        self.gripColor = UltraSheetViewController.gripColor
        self.gripSize = UltraSheetViewController.gripSize
        self.pullBarBackgroundColor = UltraSheetViewController.pullBarBackgroundColor
        self.cornerRadius = UltraSheetViewController.cornerRadius
        self.updateOrderedSizes()
    ...
    public func setSizes(_ sizes: [UltraSheetSize], animated: Bool = true) {
    ...
    func updateOrderedSizes() {
        var concreteSizes: [(UltraSheetSize, CGFloat)] = self.sizes.map {
    ...
    private func addPanGestureRecognizer() {
        let panGestureRecognizer = UltraInitialTouchPanGestureRecognizer(target: self, action: #selector(panned(_:)))
    ...
    private func height(for size: UltraSheetSize?) -> CGFloat {
    ...
    public func resize(to size: UltraSheetSize,
    ...
    public func animateIn(to view: UIView, in parent: UIViewController, size: UltraSheetSize? = nil, duration: TimeInterval = 0.3, completion: (() -> Void)? = nil) {
    ...
    public func animateIn(size: UltraSheetSize? = nil, duration: TimeInterval = 0.3, completion: (() -> Void)? = nil) {
...
extension UltraSheetViewController: UltraSheetViewDelegate {
...
extension UltraSheetViewController: UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        guard let panGestureRecognizer = gestureRecognizer as? UltraInitialTouchPanGestureRecognizer, let childScrollView = self.childScrollView, let point = panGestureRecognizer.initialTouchLocation else { return true }
        ...
        if velocity.y < 0 {
            ...
            return height(for: self.orderedSizes.last) > containerHeight && containerHeight < height(for: UltraSheetSize.fullscreen)
        } else {
            return true
        }
    }
...
extension UltraSheetViewController: UltraSheetContentViewDelegate {
    func pullBarTapped() {
...
extension UltraSheetViewController: UIViewControllerTransitioningDelegate {
